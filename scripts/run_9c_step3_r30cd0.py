"""9-C STEP 3: r30_cd0 walk-forward — 7차 baseline"""
...
print()
print(f'{LABEL} walk-forward 시작 (가장 무거움)')
print()
...
for w in windows:
    ...
    if len(test_eq) < 10:
        ...
        active = [len(s.active_pairs) for s in res.monthly_states
                  if test_start_date <= s.date <= test_end_date and s.active_pairs]
        avg_pairs = np.mean(active) if active else 0

    cache[key] = {
        'window': w['idx'],
        'return': ret, 'sharpe': sh, 'mdd': mdd,
        'test_equity': test_eq_norm,
    ...
    print(f'  W{w["idx"]:>2d} {LABEL} '
          ...
          f'snap {n_snap} pairs {avg_pairs:.1f}  '
          f'[{time.time()-t0:.1f}s]')
